...
def train_epoch(model, loader, optimizer, epoch, loss_func, args):
    ...
    for idx, batch_data in enumerate(loader):
        ...
        for b in range(B):
            ...
            feat_list_ENT.append(feats_tiled)                  # salva per similarità
        ...
        if (epoch == 0 or epoch == (args.max_epochs-1) or epoch == int(args.max_epochs * 0.5) ) and idx == 0 and args.rank == 0:
            ...
            sim = compute_similarity_matrix(feat_list_ENT).cuda()
            sim_np = sim.detach().float().cpu().numpy()  # solo per plotting
            plot_similarity_heatmap_new(sim_np, target, save_path=os.path.join(args.sim_plots_dir, f"similarity_epoch{epoch}_iter{idx}.png"))
        ...
        if args.similarity_loss == "contrastive":
            loss_supcon = supervised_contrastive_from_similarity(sim, target, temperature=0.07)
            loss = loss_func(logits, target) + args.similarity_loss_weight * loss_supcon
            print(f"Sim loss: {loss_supcon.item()}")
        elif args.similarity_loss == "margin":
            loss_margin = similarity_margin_loss(sim, target, pos_margin=0.5, neg_margin=0.0)
            loss = loss_func(logits, target) + args.similarity_loss_weight * loss_margin
            print(f"Sim loss: {loss_margin.item()}")
        else:
        ...
        if args.rank == 0:
            print(
                ...
                "sim_loss: {:.4f}".format(loss_supcon.item() if args.similarity_loss == "contrastive" else loss_margin.item()),
...
def run_training(
        ...
        if (epoch + 1) % args.val_every == 0:
            ...
            if args.rank == 0:
                ...
                if val_avg_acc > val_acc_max:
                    ...
                    plot_metrics_table(metrics,class_names=[f"class {i}" for i in range(cm.shape[0])],title=f'Metrics Table - Epoch {epoch}', save_path=os.path.join(cm_plots_dir, f"best_metrics_table_epoch{epoch}.png"))
    ...
    plot_metrics_table(metrics,class_names=[f"class {i}" for i in range(cm.shape[0])],title=f'Metrics Table - Final Epoch', save_path=os.path.join(cm_plots_dir, f"final_metrics_table.png"))
    ...
    if args.telegram_log:
        ...
        message = f"*Metrics Table*"
        asyncio.run(send_alert(args.oar_id, message, token_file=args.token, image_path=os.path.join(cm_plots_dir, "final_metrics_table.png")))
