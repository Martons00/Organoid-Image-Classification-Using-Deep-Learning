...
def test_epoch_new(model,loader: DataLoader,epoch: int,acc_func,loss_func,args,) -> tuple[float, dict, np.ndarray]:
    """
    Testing con pipeline di inferenza a patch ottimizzata.
    ...
    if is_main_process:
        summary = ", ".join([f"c{c}: {per_class_acc[c]:.3f}" for c in range(num_classes)])
        print(f"[Testing] avg_acc={run_acc.avg:.4f} | avg_loss={run_loss.avg:.4f} | per-class [{summary}]")

    return float(run_loss.avg), float(run_acc.avg), per_class_acc, cm, all_errors_paths

def test_epoch(model,loader: DataLoader,epoch: int,acc_func,loss_func,args,) -> tuple[float, dict, np.ndarray]:
    """
    Testing con pipeline di inferenza a patch ottimizzata.
...
def run_testing(
    ...
    epoch_time = time.time()
    if args.patch_merging:
        _,test_acc, test_per_class, cm, test_errors_paths = test_epoch_new(
