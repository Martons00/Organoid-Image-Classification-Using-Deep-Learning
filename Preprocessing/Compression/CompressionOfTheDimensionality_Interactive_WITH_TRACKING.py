...
def chiedi_range_crop(max_depth, max_height, max_width):
    ...
    Versione modificata: chiede start, poi fine separatamente per ogni piano

    Args:
        max_depth: numero massimo di slice Z
        max_height: numero massimo di pixel Y  
        max_width: numero massimo di pixel X

    Returns:
        dict: {'z': (start, end), 'y': (start, end), 'x': (start, end)}
        None se l'utente annulla

    Esempio di utilizzo:
        crop_ranges = chiedi_range_crop_separato(100, 800, 600)
        if crop_ranges:
            z_start, z_end = crop_ranges['z']
            y_start, y_end = crop_ranges['y'] 
            x_start, x_end = crop_ranges['x']
    """
    print(f"\n🎯 SELEZIONE RANGE CROP (input separato)")
    ...
    print()
    print(f" 💡 Per ogni piano:")
    print(f" - Inserisci il valore di START e premi INVIO")
    print(f" - Inserisci il valore di FINE e premi INVIO")
    print(f" - Oppure scrivi 'tutto' per usare tutto il range")
    print()

    crop_ranges = {}

    def chiedi_singolo_asse(nome_asse, limite_max):
        """Helper function per richiedere un singolo asse"""
        print(f"\n📐 {nome_asse.upper()} [0-{limite_max-1}]:")

        while True:
            try:
                # Chiedi START
                start_input = input(f" 👉 {nome_asse} START [0-{limite_max-2}]: ").strip()

                # Opzione rapida 'tutto'
                if start_input.lower() in ['tutto', 'all','t']:
                    print(f" ✅ {nome_asse}: tutto il range (0-{limite_max-1})")
                    return (0, limite_max-1)
                elif  start_input.lower() in ['']:
                    start_input = 0

                start_val = int(start_input)

                # Validazione START
                if start_val < 0 or start_val >= limite_max-1:
                    print(f" ⚠️ START fuori dai limiti! Usa 0-{limite_max-2}")
                    continue

                # Chiedi FINE
                while True:
                    try:
                        end_input = input(f" 👉 {nome_asse} FINE [{start_val+1}-{limite_max-1}]: ").strip()
                        if  end_input.lower() in ['']:
                            end_input = limite_max-1
                        end_val = int(end_input)

                        # Validazione FINE
                        if end_val <= start_val:
                            print(f" ⚠️ FINE deve essere > START ({start_val})")
                            continue
                        elif end_val >= limite_max:
                            print(f" ⚠️ FINE fuori dai limiti! Usa fino a {limite_max-1}")
                            continue

                        # Valori validi - calcola dimensione
                        dimensione = end_val - start_val + 1
                        unita = 'slice' if nome_asse == 'Z' else 'pixel'
                        print(f" ✅ {nome_asse}: {start_val}-{end_val} ({dimensione} {unita})")
                        return (start_val, end_val)

                    except ValueError:
                        print(f" ⚠️ Inserisci un numero valido per FINE!")
                    except (KeyboardInterrupt, EOFError):
                        print("\n 🛑 Operazione annullata")
                        return None

            except ValueError:
                print(f" ⚠️ Inserisci un numero valido per START!")
            except (KeyboardInterrupt, EOFError):
                print("\n 🛑 Operazione annullata")
                return None
    ...
    # Richiedi ogni asse in ordine Z, Y, X
    # Z (profondità - numero di slice)
    z_range = chiedi_singolo_asse('Z', max_depth)
    if z_range is None:
        return None
    crop_ranges['z'] = z_range

    # Y (altezza - righe dell'immagine)
    y_range = chiedi_singolo_asse('Y', max_height)
    if y_range is None:
        return None
    crop_ranges['y'] = y_range

    # X (larghezza - colonne dell'immagine)
    x_range = chiedi_singolo_asse('X', max_width)
    if x_range is None:
        return None
    crop_ranges['x'] = x_range

    # Riepilogo della selezione
    print(f"\n📋 RIEPILOGO CROP SELEZIONATO:")
    z_dim = crop_ranges['z'][1] - crop_ranges['z'][0] + 1
    y_dim = crop_ranges['y'][1] - crop_ranges['y'][0] + 1  
    x_dim = crop_ranges['x'][1] - crop_ranges['x'][0] + 1

    print(f" Z: {crop_ranges['z'][0]}-{crop_ranges['z'][1]} ({z_dim} slice)")
    print(f" Y: {crop_ranges['y'][0]}-{crop_ranges['y'][1]} ({y_dim} pixel)")
    print(f" X: {crop_ranges['x'][0]}-{crop_ranges['x'][1]} ({x_dim} pixel)")
    print(f"\n 📊 Volume finale: {z_dim} × {y_dim} × {x_dim} = {z_dim * y_dim * x_dim:,} voxel")

    # Conferma finale
    while True:
        try:
            conferma = input(f"\n✅ Confermi questi valori? [S/n]: ").strip().lower()
            if conferma in ['s', 'si', 'y', 'yes', '']:
                print(" 🎯 Crop confermato!")
                return crop_ranges
            elif conferma in ['n', 'no']:
                print("🔄 Ricomincio la selezione...")
                return chiedi_range_crop_separato(max_depth, max_height, max_width)
            else:
                print(" ⚠️ Rispondi con S (si) o N (no)")
        except (KeyboardInterrupt, EOFError):
            print("\n 🛑 Operazione annullata")
            return None
...
def chiedi_range_crop_unito(max_depth, max_height, max_width):
    """
    Chiede all'utente di specificare il range di crop per X, Y, Z
    Basato su tif_z_range_selector_tifffile.py
    """
...
if __name__ == "__main__":
    ...
    input_folder = 'F:\\Organoids\\Chouxfleurs_raw'
    output_folder = 'F:\\Organoids\\Organoids_Dataset\\Chouxfleurs_raw_Nice_Reduce'
