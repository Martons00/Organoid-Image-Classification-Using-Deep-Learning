...
from sliceSelector import   SliceSelector  # Assicurati di avere questo modulo disponibile
...
def analyze_and_resize_tiff_volumes(input_folder, output_folder, target_shape=(512, 512), preserve_structure=True):
    ...
    selector = SliceSelector(method='feature_variance')
    ...
    for i, tif_path in enumerate(tif_files_str):
        try:
            ...
            D, H, W = vol_np.shape
            target_D, target_H, target_W = target_shape[0], target_shape[0], target_shape[1]
            if D > target_D:
                d_idx = selector.select_axis(vol_np, axis=0, n_slices=target_D)
                vol_np = vol_np[d_idx, :, :]
            elif D < target_D:
                pad = target_D - D
                pad_front = pad // 2
            ...
            if H > target_H:
                h_idx = selector.select_axis(vol_np, axis=1, n_slices=target_H)
                vol_np = vol_np[:, h_idx, :]
            elif H < target_H:
                pad = target_H - H
                pad_top = pad // 2
            ...
            if W > target_W:
                w_idx = selector.select_axis(vol_np, axis=2, n_slices=target_W)
                vol_np = vol_np[:, :, w_idx]
            elif W < target_W:
                pad = target_W - W
                pad_left = pad // 2
                ...
                vol_np = np.pad(vol_np, ((0, 0), (0, 0), (pad_left, pad_right)), 
                                mode='constant', constant_values=0.0)
            # Ridimensiona volume
            resized_volume = vol_np.astype(np.uint8)  # Assicurati che sia uint8 prima di salvare
...
if __name__ == "__main__":
    ...
    input_folder = '/home/mraffael/martone_project/Organoids_Dataset/train_set'
    output_folder = '/home/mraffael/martone_project/Organoids_Dataset_Slice_128/train_set'
    # MODIFICATO: Aggiunto parametro preserve_structure
    analyze_and_resize_tiff_volumes(input_folder, output_folder, target_shape=(128,128), preserve_structure=True)
